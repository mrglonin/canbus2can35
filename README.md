...
CAN, хранит маленький snapshot и держит последние штатные nav/compass кадры.
Android забирает Vehicle/RCTA/voltage через `0x77`; тот же compact poll дает
адаптеру tick для удержания `0x45` compass и active nav bundle. Raw stream
`0x70/0x76` не используется для обычных Vehicle/RCTA/nav/compass, только для
...
Текущий Android слой должен отправлять не raw CAN, а правильные USB/API события:

1. Компас: `0x45` только при смене направления; удержание `350-500 ms` делает V21.
2. Навигация: `0x48`, `0x45`, `0x47`, `0x4A`, `0x44` при изменении route state; active bundle повторяет адаптер.
