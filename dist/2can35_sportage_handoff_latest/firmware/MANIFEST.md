...
| `firmware/canlog/2can35_04350008_mode3_lab_can_uart_usb.bin` | 32176 | `d17c2fb3fd8ead6d5086260e293eb3e6ddb638db4d7af8c92464fc6248f0c7c6` | Lab package matching the intended mode split: programmer v08 mode1, stock update mode2, custom CDC mode3 at `0x08009000` with CAN RX/TX and TEYES/Raise UART RX/TX. Mode1 differs from v08 only in reset vector and two mode-switch hooks. |
...
| `firmware/custom_c/dist/2can35_custom_04351024_can_uart_sunroof_test_update.bin` | 11360 | `b0884e60c4a20cf0bf21287a986229d54252b05868de47d592ce168ca696698e` | Clean C experimental USB update package. CAN HW + reverse output + TEYES/Raise UART enabled. Adds experimental `0x40` sunroof bit to Raise body frame `FD 05 05 XX 00 CS`, generated from C-CAN `0x541 DATA[7] & 0x02`. |
| `/Users/legion/Downloads/2CAN35_CUSTOM_CAN_UART_SUNROOF_TEST_04351024_update.bin` | 11360 | `b0884e60c4a20cf0bf21287a986229d54252b05868de47d592ce168ca696698e` | Same local copy for quick flashing/testing. |
| `firmware/custom_c/dist/2can35_custom_04351025_lab_can_uart_rx_tx_update.bin` | 11488 | `5254ecdbc8fec85da384df93a12a45d658f55dc73e252136746ce2bd59b6d7a1` | Clean C lab update package. Boots into logical mode3/lab, logs CAN over CDC ASCII, supports raw CAN TX, TEYES/Raise UART TX (`u...`), TEYES/Raise UART RX echo (`U FD...`), reverse output, and automatic C-CAN body/reverse -> UART reporting including experimental sunroof `0x40`. |
| `/Users/legion/Downloads/2CAN35_CUSTOM_LAB_CAN_UART_RX_TX_04351025_update.bin` | 11488 | `5254ecdbc8fec85da384df93a12a45d658f55dc73e252136746ce2bd59b6d7a1` | Same local copy for quick flashing/testing through mode2 stock update path. |
...
Clean-C packages such as `04351025` are not the current car branch. They are
kept only for lab validation of USB/CAN/UART/pinout behavior. They must not be
treated as a drop-in replacement for the programmer's Sportage firmware until
all required mode1 behavior is reproduced and tested.

The exception is `firmware/canlog/2can35_04350008_mode3_lab_can_uart_usb.bin`:
it does not replace mode1 with clean C. It keeps programmer v08 as mode1 and
uses clean C only as the isolated mode3 lab payload.
