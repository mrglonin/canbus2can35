...
Запись без префикса вроде `03 00 41 00 C0 00 00 03` - это только поле
`DATA`, то есть 8 байт полезной нагрузки. Полная запись должна включать шину,
канал, тип ID, CAN ID и длину:

`C-CAN ch1 500000 STD ID 0x541 DLC 8 DATA 03 00 41 00 C0 00 00 03`

Для текущего логгера:

- `ch0 = M-CAN`, `100000`.
- `ch1 = C-CAN`, `500000`.
- `STD` = обычный 11-bit CAN ID.
- `ID 0x541` = сообщение `CGW1`.
- `DLC 8` = длина данных 8 байт.

Пример по люку:

| Полный кадр | Значение |
|---|---|
| `C-CAN ch1 500000 STD ID 0x541 DLC 8 DATA 00 00 41 00 C0 00 00 01` | люк закрыт |
| `C-CAN ch1 500000 STD ID 0x541 DLC 8 DATA 03 00 41 00 C0 00 00 01` | зажигание включено, люк закрыт |
| `C-CAN ch1 500000 STD ID 0x541 DLC 8 DATA 03 00 41 00 C0 00 00 03` | зажигание включено, люк открыт |

Декод люка сейчас: `ID 0x541`, `DATA[7] & 0x02`. Первый байт `DATA[0] = 03`
относится к зажиганию/режиму питания и не является признаком открытого люка.
...
| Люк открыт | car -> adapter | C-CAN `ch1`, `500000`, `STD ID 0x541 CGW1`, `DLC 8`, `DATA[7] bit1 mask 0x02`: `C_SunRoofOpenState`, `CF_Gway_RrSunRoofOpenState` | закрыт: `DATA ... 01`; открыт: `DATA ... 03`; `DATA[0]=03` = зажигание, не люк | `confirmed_rx` |
...
Точный список найденных Raise/RZC Korea UART команд вынесен отдельно:
`docs/RAISE_RZC_KOREA_UART_MATRIX.md`. Для этой машины кнопки руля и панели
считаем аналоговыми: основной путь не CAN, а внешний Raise canbox, который
передает key frames по UART.
