...
def req(cmd: int, payload: bytes = b"") -> bytes:
    frame = bytearray([0xBB, 0x41, 0xA1, len(payload) + 6, cmd])
...
def main() -> int:
    ...
    assert version_req.hex(" ").upper() == "BB 41 A1 06 56 F9"
    assert mode3_req.hex(" ").upper() == "BB 41 A1 07 51 03 F8"
    assert raw_tx_req[0:5] == bytes([0xBB, 0x41, 0xA1, 0x12, 0x70])

    sample_resp = bytes.fromhex(
        "BB A1 41 18 56 37 FF DA 05 42 47 30 38 59 41 22 43 04 35 10 00 01 00"
    )
