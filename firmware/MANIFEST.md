...
| `firmware/canlog/2can35_04350004_canlog_v4_no_auto_media_usb.bin` | 32176 | `339a35e3c46ce7b906bb05046b2a97cc27ebe41edb98a74e46518d8d780864f0` | Flashed on 2026-05-07 for car testing. Same v04+mode3 package, but disables the internal mode1 media/source fallback schedulers at `0x08005948`, `0x08005A40`, `0x08005A9C`, `0x08005AF8`, `0x08005B74`. UART and explicit USB/APK display commands remain enabled. |
| `firmware/canlog/2can35_04350004_canlog_v4_mode3_original_tbb_no_fallback_usb.bin` | 32176 | `6fc42aa40c26e7009bd327aff1637d56c5707616a6a51de7483e43bf125fcea8` | Current candidate flashed after `no_auto_media` broke source/compass. Keeps v4-final mode3/software switching, restores original media scheduler TBB states 4/5/6 (`3D 49 4D`) for source/compass, and NOPs only fallback dispatcher branches at `0x08005B7A`, `0x08005B90`, `0x08005B9A`. |
...
Use `firmware/canlog/2can35_04350006_canlog_v4_mode3_preserve_beeps_usb.bin`
when testing the programmer's v06 behavior with our three-mode wrapper. It keeps
normal canbox mode as the programmer's v06 update, preserves the existing
...
The v06 package does not carry the old v04 media NOP patch. Media/BL/parking
logic is left to the programmer's v06 implementation; only the local mode
switch hooks and reset hook are added.

The broader `no_auto_media` package removed the spam but also disabled source
switching and the default compass path, so keep it only as a diagnostic
reference.
