...
3. Связать CAN IDs из лога с событиями: двери, ACC, задний ход, скорость, обороты, климат, TPMS. Кнопки руля и панельные "пианино" кнопки для этой конфигурации считаем аналоговыми и ведем через stock Raise canbox -> UART2 -> наш адаптер -> магнитола.
