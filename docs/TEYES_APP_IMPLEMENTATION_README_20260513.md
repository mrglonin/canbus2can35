...
CC4PRO, V20 2CAN35 и Sportage CAN-C/M-CAN.
...
- `vehicle` - whitelist CAN IDs из пассивного потока.
...
Raw CAN stream - рабочий пассивный listener, не debug сам по себе. Он включается,
если включены Vehicle или RCTA, но APK обрабатывает только whitelist:
...
Все остальные кадры отбрасываются до логов/UI, если debug CAN выключен.
...
Напряжение берется из M-CAN `0x132 DATA[0] / 10`. Старый `0x545` остается только
historical fallback из V18 snapshot.
