...
Цель следующей рабочей прошивки: перенести удержание штатных событий в
адаптер, чтобы APK не слал постоянные повторы и не использовал raw M-CAN как
основной способ работы. APK должен сообщать только факт изменения состояния,
а адаптер уже сам держит маленькое состояние и отдает его штатному parser.
...
|---|---|---|---|
| Компас | новый compass value через штатный `0x45` payload | хранит последний valid compass и повторяет `350-500 ms`; формула панели `sent = (36 - ui) % 36` | нет |
| Навигация | route state changes: `0x7A nav`, `0x48`, `0x45`, `0x47`, `0x4A`, `0x44` | хранит route bundle, повторяет active route около `1 s`, finish hold около `5 s`, чистый off без blink | нет |
